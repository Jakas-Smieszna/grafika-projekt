...
void UpdateGameState(GameElements game, GLFWwindow* window) 
{
	...
	if (i < 102.0f)
		game.lightColor = glm::vec4(0.f, 0.f, i * 0.02f, 1.0f);
	else
		game.lightColor = glm::vec4(0.f, 0.f, 2.0f - (i - 100.0f) * 0.02f, 1.0f);

	game.light2Model = glm::translate(game.light2Model, -game.light2Pos);
	game.light2Pos = glm::rotate(game.light2Pos,
		glm::radians(0.25f),
...
void RenderScene(GameElements game, GLFWwindow* window) {
	...
	glUniform4f(glGetUniformLocation(game.shaderProgram.ID, "lightColor"),
		game.lightColor.x, game.lightColor.y,
		game.lightColor.z, game.lightColor.w);
